...
export interface AirlockHeroProps {
    /** Video to scrub. Must be same-origin or CORS-enabled, and seekable. Pass null for a blank screen (the scroll mechanic still runs). */
...
export default function AirlockHero({
    ...
    useEffect(() => {
        ...
        // Integration fix: with server rendering the <video> can finish loading
        // before hydration attaches the listener above, so "loadeddata" is missed
        // and the hero never seeks or fades the video in. Catch up if it's loaded.
        if (video.readyState >= 2 || !video.getAttribute("src")) onLoadedData()
